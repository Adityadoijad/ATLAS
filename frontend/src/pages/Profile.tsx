...
function buildProfileFromAuthUser(authUser: { name: string; email: string } | null) {
  return {
...
export function ProfilePage() {
  ...
  const save = () => {
    if (!draft.fullName.trim() || !/^\S+@\S+\.\S+$/.test(draft.email)) {
      toast({ title: t('profile.invalid'), description: 'A name and valid email are required.', tone: 'error' });
      return;
    }
    setProfile(draft);
    setEditing(false);
      ...
      <div className="grid gap-5 lg:grid-cols-[300px_1fr]">
        <Card className="h-fit p-6 text-center">
          ...
          <Button
            ...
            {editing ? t('profile.cancelEditing') : t('profile.editProfile')}
          </Button>
        </Card>

        <div className="space-y-5">
          <Card className="p-6">
            <h2 className="text-[15px] font-bold text-ink">{t('profile.accountInfo')}</h2>
            {editing ?
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <Field label={t('profile.fullName')} htmlFor="p-name">
                  <Input id="p-name" value={draft.fullName} onChange={(e) => setDraft({ ...draft, fullName: e.target.value })} />
                </Field>
                <Field label={t('profile.emailAddress')} htmlFor="p-email">
                ...
                <Field label={t('profile.nationality')} htmlFor="p-nat">
            ...
            <dl className="mt-4 divide-y divide-line">
                {[
              [t('profile.fullName'), profile.fullName], [t('profile.emailAddress'), profile.email], [t('profile.phone'), profile.phone], [t('profile.dob'), profile.dob], [t('profile.nationality'), profile.nationality]].
              map(([label, value]) =>
          ...
          <Card className="p-6">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <Field label={t('profile.preferredLanguage')} htmlFor="tp-lang">
              ...
              <Field label={t('profile.foodPreference')} htmlFor="tp-food">
              ...
              <Field label={t('profile.accommodation')} htmlFor="tp-acc">
              ...
              <Field label={t('profile.transport')} htmlFor="tp-tr">
              ...
              <Field label={t('profile.budget')} htmlFor="tp-bud">
              ...
              <Field label={t('profile.accessibility')} htmlFor="tp-a11y">
